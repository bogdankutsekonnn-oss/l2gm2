...
const { data: article } = await useAsyncData(`blog-${slugParam}`, () =>
...
const relatedArticles = computed(() => {
  const list = (allArticles.value || []).filter((a) => a.slug !== slugParam)
...
// Prev / Next навигация (по дате, сортировка DESC уже сделана в запросе)
const currentIndex = computed(() =>
  (allArticles.value || []).findIndex((a) => a.slug === slugParam)
)
// allArticles отсортирован DESC: [самая_свежая, ..., самая_старая]
// "Следующая" = более свежая = индекс на один меньше
...
// --- AST утилиты для подсчёта слов и сбора заголовков ---

// Рекурсивно вытаскивает весь текст из AST Content v3
...
// Cлагифицирует строку (русский транслит простой — для фолбэка)
const cyrMap = {
...
function collectH2(node, out = []) {
  ...
  if (Array.isArray(node)) {
    if (typeof node[0] === 'string' && node[0].toLowerCase() === 'h2') {
      ...
      // Всё равно спускаемся вниз на случай вложенных h2
    }
...
// 200 слов в минуту — средняя скорость чтения русского текста
...
<style>
/* Прогресс-бар чтения — вне scoped, т.к. телепортится в <body> */
.reading-progress {
  position: fixed;
  top: 70px; /* Под шапкой */
  left: 0;
  ...
  z-index: 101; /* Чуть выше шапки (z-index: 100) */
  pointer-events: none;
}
