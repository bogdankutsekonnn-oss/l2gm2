...
const faqItems = [
  ...
  },
  {
    ...
    answer:
      'Мы предлагаем несколько форматов: <strong>Бесплатный</strong> — базовая карточка в общем списке (нужна кнопка на сайте). <strong>B-Grade</strong> — выделенная карточка в общем списке. <strong>A-Grade</strong> — приоритетная позиция сразу после S-Grade серверов. <strong>S-Grade</strong> — максимальная видимость с анимированной карточкой и первыми местами в списке. Подробности на странице <a href="/placement/">Размещение</a>.',
  },
  {
    question: 'Кому подойдёт S-Grade или A-Grade размещение?',
    answer:
      'S-Grade и A-Grade подойдут серверам, которые хотят <strong>максимальный охват</strong> на старте. Выделенное оформление карточки и приоритетная позиция в списке значительно увеличивают количество переходов. Это особенно эффективно для серверов, открывающихся в ближайшие дни.',
  },
  {
...
L2GM помогает игрокам быстро находить новые сервера по хроникам, рейтам и дате старта. Для администраторов серверов доступны бесплатные и платные форматы размещения с разным уровнем видимости и приоритета.
