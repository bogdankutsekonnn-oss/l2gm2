...
export const useSeo = () => {
  ...
  const generateDescription = (filters = {}) => {
    ...
    return 'Новые сервера Lineage 2 2026 с датами старта. Топ серверов Л2 всех хроник и рейтов — Interlude, High Five, Essence. Выбирай лучший проект на L2GM!'
  }
  ...
  const chronicleDescriptions = {
    ...
    'c1': 'C1 (Harbingers of War) — самая ранняя из доступных хроник Lineage 2. Нет третьей профессии, нет ноблесс, минимум контента и максимум хардкора. Прокачка медленная, экипировка дорогая, каждое достижение ощущается остро. Чистый олдскул для ветеранов, которые знают игру с 2004 года.',
  ...
  const generateSeoText = (filters = {}) => {
    ...
    // Уникальный лонгрид для лендинга рейта (без хроники) — есть у ходовых
    // рейтов (x1, x50, x100, x1200), остальные остаются на шаблоне
    if (rate && !filters.chronicle && rate.seoText) {
      return rate.seoText
    }
    ...
    // Блок про качество
    paragraphs.push(`В верхней части списка закреплены S-Grade сервера — проекты с подтверждённой репутацией, стабильным онлайном и активным сообществом. Выбирая сервер с S-Grade статусом, вы получаете гарантию качественной игры.`)
  ...
  // Уникальный лонгрид для главной страницы.
  // Не использует шаблонные абзацы generateSeoText, чтобы главная не дублировала
  // тексты страниц рейтов. Написания «л2», «ла2», «линейдж 2» подмешаны
  // по данным Wordstat (л2 — 59% спроса, ла2 — 14%, линейдж 2 — 5%).
  const generateHomeSeoText = () => {
    const paragraphs = [
      `L2GM — это список новых серверов Lineage 2 с точными датами старта и удобными фильтрами. Мы ежедневно обновляем каталог серверов Л2, чтобы вы видели только живые проекты: свежие анонсы, сервера ближайших дней и недавно открывшиеся миры. Вместо десятков форумов и групп — один актуальный рейтинг, где собрано всё важное о каждом проекте.`,

      `На сайте представлены сервера всех хроник: легендарный Interlude (Интерлюд) — золотой стандарт PvP, High Five (Хай Файв) — максимум классического контента, хардкорный Classic (Классик), динамичный Essence (Эссенс) с авто-охотой, а также олдскульные C4 и сборки с дополнениями. У каждой хроники — отдельный раздел с собственным списком серверов и календарём открытий.`,

      `Рейты — на любой стиль игры. Лоу рейт x1–x3 подойдёт тем, кто ценит долгую прокачку и стабильную экономику; мид рейт x50–x100 — золотая середина между хардкором и комфортом; PvP-сервера x1200+ дают мгновенный старт в эндгейм. В карточке каждого сервера ла2 указаны хроника, рейты, дата открытия и ключевые особенности проекта.`,

      `Как выбрать лучший сервер? Ориентируйтесь на дату старта: новые сервера Л2, открывающиеся сегодня или завтра, собирают максимальный онлайн — заходить в первый день выгоднее всего. Проекты со статусом S-Grade и A-Grade закреплены в топе списка: это сервера с подтверждённой репутацией и стабильным онлайном.`,

      `Линейдж 2 живёт уже третье десятилетие, и каждую неделю стартуют новые проекты — русскоязычные и зарубежные, классические PvE, PvP и GvE сервера, мультипрофа, мультикрафт и кастомные концепции. Календарь открытий поможет спланировать возвращение в игру, а новости и блог L2GM — быть в курсе событий мира Lineage 2.`,

      `Добавьте L2GM в закладки: список обновляется каждый день, и анонсы новых серверов появляются здесь раньше, чем о них узнаёт большинство игроков. Удачного старта!`,
    ]
    return paragraphs.join('\n\n')
  }
  ...
  // Без potentialAction/SearchAction: страницы /search на сайте нет,
  // а несуществующая разметка подрывает доверие к структурированным данным.
  const generateHomeJsonLd = () => {
    return {
      '@context': 'https://schema.org',
      '@type': 'WebSite',
      name: 'L2GM',
      description: 'Новые сервера Lineage 2 всех хроник и рейтов с датами старта',
      url: siteUrl,
      inLanguage: 'ru',
    }
  }
  ...
  const generateImageJsonLd = () => {
    return {
      ...
      width: 1200,
      height: 630,
      caption: 'L2GM — Сервера Lineage 2'
    }
  }
  ...
  const chronicleFaqDescriptions = {
    ...
    'c1': 'C1 (Harbingers of War) — самая ранняя хроника Lineage 2 без третьей профессии и ноблов, максимально хардкорный олдскул.',
  ...
  const generateFaqItems = (filters = {}) => {
    ...
    if (chronicle && rate) {
      ...
      items.push({
        ...
        answer: `Обратите внимание на серверы со статусом S-Grade и A-Grade — это проекты с подтверждённой репутацией. Также смотрите на дату старта: серверы, открывающиеся в ближайшие дни, обычно собирают максимальный онлайн.`
      })
      items.push({
      ...
      items.push({
        ...
        answer: `Перейдите на страницу «Добавить сервер» и заполните форму. Базовое размещение бесплатное. Для выделения в списке доступны статусы S-Grade, A-Grade и B-Grade — подробности на странице «Реклама».`
      })
      if (chronicleNameRu) {
        items.push({
      ...
      items.push({
        question: 'Что такое S-Grade и A-Grade статус серверов на L2GM?',
        answer: 'S-Grade, A-Grade и B-Grade — это статусы платного размещения. Серверы с этими статусами закреплены в верхней части списка и выделены визуально. Это проекты с подтверждённой репутацией и стабильным онлайном.'
      })
      items.push({
        question: 'Как добавить свой сервер Lineage 2 на L2GM?',
        answer: 'Перейдите на страницу «Добавить сервер», заполните форму с данными о проекте. Базовое размещение бесплатное. Для выделения в списке доступны статусы S-Grade, A-Grade и B-Grade (от $3 за 7 дней) — подробности на странице «Реклама».'
      })
    }
